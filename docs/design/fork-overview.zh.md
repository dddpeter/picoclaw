...
| 长任务第三批：卡死三层防护 | `<2026-09-20>` | `pkg/agent/progress_heartbeat.go`（stall watchdog）、`pkg/channels/pico/protocol.go`+`pico.go`（progress_note kind）、`web/frontend/src/features/chat/use-turn-stall.ts`（前端卡死横幅） | 本文 §12、`docs/design/long-task-execution.zh.md` §9 |
...
- **出口预算兼底（2026-09-21，架构体检第 4 项）**：`pkg/tools/output_budget.go` 的 `ApplyOutputBudget` 在 registry 同步出口（`ExecuteWithContext` normalize 之后）与 async 回调（`pipeline_execute_loop.go`）两条路对 `ForLLM` 施加最后兼底截断（保尾 + UTF-8 边界安全 + `[output truncated: kept the last N of M bytes]` 提示），配置 `tools.max_tool_output_bytes`（0=默认 128KB，负=无限；默认刻意高于 read_file 64KB/shell 50KB，内置工具永不触发、无双层提示，纯拀 MCP/第三方无预算输出）。ForUser 不动（走聊天侧不进上下文）。同批：`ExecuteWithContext` 的 Info 级全量 args 改 200 字预览（shell 命令带 token 落盘问题）。测试锚点：`TestApplyOutputBudget_*`、`TestExecuteWithContext_AppliesOutputBudget`。
...
- **MiniMax 工具调用泄漏清洗（2026-09-24，飞书流式卡片乱码事故驱动）**：MiniMax 模型侧已知缺陷（上游 trackers：`MiniMax-AI/MiniMax-M3#31`、`vllm#51073`、openclaw PR #126307）——上游工具调用解析偶发失败时，原生 XML 标记连同编码边界 token `]<]minimax[>[` 一起泄漏进 `delta.content`（如 `PYEOF]<]minimax[>[</command>]<]minimax[>[</invoke>]<]minimax[>[</tool_call>`），参数体还可能携带密钥（本次事故把 hqai key 推上了飞书卡片）。fork 修复：`pkg/providers/openai_compat/minimax_clean.go` 的 `sanitizeMinimaxToolLeak`——**证据触发而非模型名门控**（文本含边界 token 才动手，其他 provider 永不受影响）：删全部边界 token；若删后文本以裸闭合标签链结尾，则从最外层开标签（`<tool_call>`/`<minimax:tool_call`，退化到 `<invoke`/seahorse 历史格式学舌 `[tool_use: `）整块截断——参数体不留在可见文本里。挂载点：流式 onChunk 累积帧（`parseStreamResponse` 两处）+ 流式最终 Content + 非流式 `Chat` 出口（`splitInlineThink` 后），清洗发生时 WarnCF 记录 orig/clean 长度。代码围栏不豁免（引用泄漏原文的场景远比泄漏本身罕见）。测试锚点：`TestSanitizeMinimaxToolLeak*`、`TestProviderChatStream_StripsMinimaxToolLeak`（边界 token 跨 delta 拆开仍净）、`TestProviderChat_StripsMinimaxToolLeak`。
...
- **第三批：卡死三层防护（2026-09-20，web「生成中」永久挂死事故驱动）**：心跳只提示不收敛，且 progress_note 被 web 前端误当最终回复。三层修复：
  - **B 后端 stall watchdog（治本）**：`agents.defaults.progress_stall_interrupt_seconds`（默认 600，0=关）。心跳 goroutine 内两级升级，判定不受心跳节流约束、无需可投递面：① idle ≥ 阈值 → `requestGracefulInterrupt` + `cancelProviderCall`（`markStallInterrupted` CAS 保证通知恰好一条），turn 在迭代边界收尾总结；② interrupt 后再静默一个心跳间隔 → `requestHardAbort`（turnCancel 解开无视取消的工具）、abort 原因 `stall_watchdog` 与 abort 标志在同一临界区写入（`requestHardAbortWithReason`，避免 turn 被唤醒后读到空原因），通知走脱离 turnCtx 的 5s 超时上下文。阈值自动钳到 ≥ 2× 心跳间隔防误配。与 §5 `watchHardAbortUnwind` 互补：watchdog 主动发现卡死，unwind 兜底释放会话。心跳文案同步升级（idle ≥ 2×interval 显示"⚠️ 已 X 无新进展……持续无响应时将自动收尾本轮"）。
  - **C progress_note 全链路 kind 标记**：pico `Send` 打 `kind: "progress_note"`（`protocol.go` 新常量），且 `outboundMessageFinalizesTrackedToolFeedback` 排除该 kind（不提前定格 tool-feedback 气泡）；web 前端 `AssistantMessageKind` 增补 `progress_note`——不再误清 isTyping 造成"假完成"。
  - **A web 前端卡死提示（看得见）**：chat store 增 `lastTurnActivityAt`（message.create/update、typing 事件均为服务端活动，含心跳拍；turn 结束/断连/切会话即清理）；`use-turn-stall.ts` 完全无事件 5 分钟 → 消息列表底部琥珀色横幅（i18n zh/en）。分层：正常慢任务心跳 3 分钟一拍持续刷新活动时间戳不误报；收尾/中止由后端 watchdog（600s）负责。
  - 测试锚点：`TestProgressHeartbeat_StallWatchdogInterrupts`、`TestProgressHeartbeat_StallWatchdogEscalatesToHardAbort`、`TestProgressHeartbeat_StallWatchdogSilentWhenActive`、`TestProgressHeartbeat_OutboundFallbackWithoutStreamer`（kind 断言）。
