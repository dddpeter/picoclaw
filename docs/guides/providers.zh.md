...
同时会对每个候选应用 cooldown（指数退避：1min → 5min → 25min → 1h；配额耗尽 5h 起），避免对刚失败的目标立即重试。

两个 fork 行为保证回合不会因冷却而死：

1. **冷却耗尽不再判死回合**：所有候选都在冷却中时，fallback 链会强制尝试「最快恢复」的候选（旧行为下，聚合网关瞬时抖动把全部候选打入冷却后，冷却窗口内后续回合会直接失败）。任一次成功即清空冷却，下一回合恢复正常。
2. **冷却可整体关闭**：`agents.defaults.cooldown_enabled` 设为 `false` 后，失败不设冷却、候选永不被冷却跳过（始终尝试语义）。第 1 条的强制尝试与该开关无关，始终生效。
