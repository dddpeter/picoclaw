...
1. **候选全部冷却时不再直接判死回合**：fallback 链会强制尝试「最快恢复」的那个候选——冷却只用来优先选健康候选，不会把仅剩的选择全部堵死（旧行为下，聚合网关的瞬时抖动把全部候选打入冷却后，冷却窗口内后续回合会直接失败）。
2. **可整体关闭**：`agents.defaults.cooldown_enabled` 设为 `false` 后，冷却机制完全不生效（失败不设冷却、候选永不被冷却跳过，回到始终尝试语义）。
...
| 字段 | 默认 | 说明 |
|---|---|---|
| `cooldown_enabled` | 未配置时视为 `true` | `false` = 关闭冷却机制（始终尝试所有候选）。冷却全跳过时的强制尝试（第 1 条）与该开关无关，始终生效 |

> 建议：先用默认配置观察——冷却全跳过强制尝试已解决「冷却窗口内回合死掉」的问题；只有仍不希望任何候选被冷却跳过时才关掉它。
